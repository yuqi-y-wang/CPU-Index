...
from mysurvival import HAS_GPU
from mysurvival import utils
from mysurvival.utils import neural_networks as nn
from mysurvival.utils import optimization as opt
...
class BaseMLPModel(BaseModel):
    ...
    def __init__(self, structure, bins = 100, auto_scaler=True):
        ...
        super(BaseMLPModel, self).__init__(auto_scaler)
    ...
    def fit(self, X, T, E, init_method = 'glorot_uniform', optimizer ='adam', 
        ...
        * self : object
        ...
        model, loss_values, error = opt.optimize(self.loss_function, model, optimizer, 
    ...
    def predict_risk(self, x, use_log=False):
        ...
        * `use_log`: **bool** *(default=True)* -- 
        ...
        risk = super(BaseMLPModel, self).predict_risk(x)
        if use_log:
            return np.log(risk)
        else:
            return risk
...
class MLP_reg_Model(BaseMLPModel):
    ...
    * `auto_scaler`: **boolean** *(default=True)* -- 
    ...
    def __init__(self, structure, bins = 100, auto_scaler = True):
        ...
        super(MLP_reg_Model, self).__init__(
    ...
    def __repr__(self):
        ...
        if self.structure is None:
            super(MLP_reg_Model, self).__repr__()
            return self.name
